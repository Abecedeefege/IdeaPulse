...
export default async function DashboardPage() {
  ...
  return (
    <div>
      ...
      {byBatch.length === 0 ? (
        <p className="text-zinc-500">No batches yet. <Link href="/onboarding" className="text-violet-400 hover:text-violet-300">Sign up</Link> to get your first 10 ideas.</p>
      ) : (
                ...
                {batch.ideas.map((idea) => {
                  ...
                  return (
                    <li key={idea.id} className="border border-zinc-800 rounded-xl p-4 bg-zinc-900/40 hover:border-zinc-700 transition-colors">
                      ...
                      <Link href={`/idea/${idea.id}`} className="text-sm text-violet-400 hover:text-violet-300 mt-2 inline-block">View idea →</Link>
                    </li>
                  );
                })}
              </ul>
