...
export function buildIdeasPrompt(userProfileSummary: string, count: number = 5, ideaProfile?: string): string {
  let prompt = IDEAS_PROMPT_TEMPLATE
    .replace("{{count}}", String(count))
    .replace("{{userProfileSummary}}", userProfileSummary || "general audience");

  if (ideaProfile) {
    prompt += ` User preference profile: ${ideaProfile}`;
  }
