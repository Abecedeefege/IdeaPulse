...
/**
 * Returns the current Supabase Auth user if session exists.
 * When BYPASS_AUTH=1, falls back to BYPASS_AUTH_EMAIL (or pulse@itamoa.com) for build mode.
 * When BYPASS_AUTH is unset, returns null when no session (real auth only).
 */
